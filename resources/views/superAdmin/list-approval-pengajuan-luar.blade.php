            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex align-items-center border-bottom">
                        <h6 class="card-title mb-3">Approval Pengajuan Luar</h6>
                    </div>
                    <div>
                        <p class="mb-0 mt-3">Nama Nasabah : {{ $pengajuan->nasabahLuar->nama_lengkap }}</p>
                        <p class="mb-0">Status Pengajuan : {{ Str::title($pengajuan->status_pengajuan) }}</p>
                    </div>
                    <div class="table-responsive mt-2">
                        <table id="myTable" class="table table-striped nowrap" style="width:100%;">
                            ...
                            <tbody>
                                @foreach ($pengajuan->approval as $data)
                                    <tr>
                                        ...
                                        <td>
                                            <form action="{{ route('superAdmin.list.approval.luar.update', $data->id) }}"
                                                method="POST">
                                                @csrf
                                                @method('PUT')
                                        ...
                                        <td>
                                            <div class="btn btn-group">
                                                <button type="button" class="btn btn-gradient-danger btn-sm"
                                                    onclick="showDeleteModal('{{ route('superAdmin.list.approval.luar.delete', $data->id) }}')">Hapus</button>
                                                <button class="btn btn-gradient-warning btn-sm" data-bs-toggle="modal"
                                                    data-bs-target="#editKeteranganModal"
                                                    data-keterangan="{{ $data->catatan }}"
                                                    data-action="{{ route('superAdmin.list.approval.luar.update', $data->id) }}">
                                                    Edit
                                                </button>
                                            </div>
                                        </td>
