...
@section('page-title', 'Notifikasi Credit Analyst')
            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex align-items-center justify-content-between border-bottom">
                        ...
                        <form action="{{ route('creditAnalyst.notifikasi.read-all') }}" method="POST">
                            @csrf
                            @method('PATCH')
                                ...
                                @forelse ($notif as $item)
                                    <tr class="{{ $item->read ? 'table-white' : 'table-warning' }}">
                                        ...
                                        <td>
                                            @if (!$item->read)
                                                <!-- Tombol Mark as Read -->
                                                <form action="{{ route('creditAnalyst.notifikasi.read', $item->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PATCH')
